...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $activity = "เพิ่มรายการ!";

    try {
        // เพิ่มรายการลงในฐานข้อมูลพร้อม user_id
        $stmt = $pdo->prepare("INSERT INTO entries (description, amount, type, user_id) VALUES (?, ?, ?, ?)");
        ...
        // เก็บสถานะการเพิ่มข้อมูลสำเร็จ
        $_SESSION['modal_message'] = 'success';
    } catch (PDOException $e) {
        // เก็บสถานะการเพิ่มข้อมูลไม่สำเร็จ
        $_SESSION['modal_message'] = 'error';
    }
...
<body class="bg-gray-100">
    ...
    <div id="success-modal" class="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50 hidden">
        <div id="modal-content" class="bg-white p-8 rounded-lg shadow-xl w-1/3">
            <div class="flex justify-center mb-4">
                <!-- Checkmark SVG -->
                <svg xmlns="http://www.w3.org/2000/svg" class="checkmark" viewBox="0 0 24 24" fill="none"
                    stroke="currentColor">
                    <path d="M5 13l4 4L19 7" />
                </svg>
            </div>
            <h3 class="text-xl font-semibold mb-4 text-center text-green-600">อัพเดทสำเร็จ</h3>
            <p class="mb-4 text-center text-gray-700">ข้อมูลของคุณถูกเพิ่มเรียบร้อยแล้ว</p>
    ...
    <div id="error-modal" class="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50 hidden">
        <div id="modal-content" class="bg-white p-8 rounded-lg shadow-xl w-1/3">
            <div class="flex justify-center mb-4">
                <!-- กากบาท (Cross) SVG -->
                <svg xmlns="http://www.w3.org/2000/svg" class="svg-error" fill="none" viewBox="0 0 24 24"
                    stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                </svg>
            </div>
            <h3 class="text-xl font-semibold mb-4 text-center text-red-600">เกิดข้อผิดพลาด</h3>
            <p class="mb-4 text-center text-gray-700">การเพิ่มข้อมูลไม่สำเร็จ กรุณาลองใหม่อีกครั้ง</p>
            <div class="flex justify-center">
                <button onclick="window.location.href = 'add_entry.php';"
    ...
    <div class="container mx-auto my-8 p-6 bg-white shadow-lg rounded-lg">
        <h1 class="text-2xl font-bold text-center mb-4">เพิ่มรายการ</h1>
        ...
        <form action="add_entry.php" method="POST">
            ...
            <button type="submit" class="bg-blue-500 text-white py-2 px-4 rounded">บันทึก</button>
            <a href="index.php">
    ...
    <script>
        // ตรวจสอบว่าเรามีข้อความจาก session หรือไม่
        <?php if (isset($_SESSION['modal_message'])): ?>
            const modalMessage = "<?php echo $_SESSION['modal_message']; ?>";

            // แสดง modal ตามสถานะ
            if (modalMessage === "success") {
                document.getElementById("success-modal").classList.add("show");
                setTimeout(function () {
                    window.location.href = "index.php"; // ไปที่หน้า index หลังจาก 3 วินาที
                }, 3000);
            } else if (modalMessage === "error") {
                document.getElementById("error-modal").classList.add("show");
            }

            // ลบ session หลังจากใช้งาน
